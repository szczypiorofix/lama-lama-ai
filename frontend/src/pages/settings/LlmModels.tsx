...
export function LlmModels() {
        ...
        if (pullImage) {
            const startModelDownload = (modelName: string) => {
                ...
                const foundLlmImage: LlmImage | null =
                    state.llms.find((llm) => llm.name === imageNameParts[0] && llm.version === imageNameParts[1]) ||
                    null;
    ...
    useEffect(() => {
        function startDownloadTask(name: string) {
            ...
            const eventSource = new EventSource(`${API_BASE_URL}/models/pull/${name}/stream`);
            ...
            eventSource.onmessage = (event: MessageEvent<string>) => {
                ...
                if (event.data.startsWith('{')) {
                    ...
                    backgroundTask.progress = Math.round((completed * 100) / total);
                } else {
        ...
        if (state.backgroundTask?.status === BackgroundTaskStatusEnum.RUNNING) {
            const image: LlmImage = state.backgroundTask?.taskObject as LlmImage;
    ...
    const ModelListItem = (props: { image: LlmImage; isLast: boolean; downloading: boolean; progress: number }) => {
        const imageFullName: string = props.image.name + ':' + props.image.version;
                ...
                <ListItem sx={{ pt: 0, pb: 0, pl: 1, pr: 1 }}>
                    <Box sx={{ minWidth: '64px' }}>
                        {!props.image.downloaded ? (
                            ...
                            <IconButton disabled={pullImage !== null} onClick={() => setDeleteImage(imageFullName)}>
                    ...
                    <ListItemText
                        ...
                        sx={{ minWidth: '132px' }}
                    />
                    {props.downloading && <ProgressBar progress={props.progress} />}
                </ListItem>
        ...
        <Box>
            ...
            <Box mb={1} mt={1}>
                <Button
                    ...
                    loading={!updated || loading || pullImage !== null}
                ...
                <List dense={false} sx={{ mb: 1 }}>
                    {state.llms.map((modelItem, index) => (
                        <ModelListItem
                            ...
                            progress={state.backgroundTask?.progress || 0}
                            downloading={
                                modelItem.id === (state.backgroundTask?.taskObject as LlmImage)?.id &&
                                (state.backgroundTask?.status === BackgroundTaskStatusEnum.RUNNING ||
                                    state.backgroundTask?.status === BackgroundTaskStatusEnum.SUSPENDED)
                            }
                        />
                    ))}
                </List>
